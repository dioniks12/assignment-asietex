...
@section('page_title', __('Tambah Pemasok'))
...
<div class="max-w-xl mx-auto space-y-6">
    ...
    <!-- Header Actions -->
    <div>
        <h3 class="text-lg font-bold text-slate-800 dark:text-white">{{ __('Tambah Pemasok Baru') }}</h3>
        <p class="text-sm text-slate-500 dark:text-slate-400">{{ __('Tambahkan mitra vendor bahan baku baru ke dalam direktori.') }}</p>
    </div>
    ...
    <div class="bg-white dark:bg-slate-900 border border-slate-200 dark:border-slate-800 rounded-3xl p-6 lg:p-8 shadow-md dark:shadow-xl transition-colors duration-200">
        <form action="{{ route('suppliers.store') }}" method="POST" class="space-y-6" data-confirm="{{ __('Apakah Anda yakin data pemasok ini sudah benar?') }}">
            @csrf
            ...
            <div>
                ...
                <div class="mt-2 relative">
                    <textarea id="address" name="address" rows="4" required
                              ...
                              placeholder="{{ __('Contoh: Jl. Industri Raya No. 45, Bandung, Jawa Barat') }}">{{ old('address') }}</textarea>
                </div>
                @error('address')
                    <p class="mt-2 text-xs text-rose-500 dark:text-rose-400 font-medium">{{ $message }}</p>
                @enderror
