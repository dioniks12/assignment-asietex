...
@section('page_title', 'Ubah Pemasok')
...
<div class="max-w-xl mx-auto space-y-6">
    ...
    <!-- Header Actions -->
    <div>
        <h3 class="text-lg font-bold text-slate-800 dark:text-white">Ubah Pemasok</h3>
        <p class="text-sm text-slate-500 dark:text-slate-400">Perbarui informasi kontak dan alamat untuk Pemasok: {{ $supplier->name }}.</p>
    </div>
    ...
    <div class="bg-white dark:bg-slate-900 border border-slate-200 dark:border-slate-800 rounded-3xl p-6 lg:p-8 shadow-md dark:shadow-xl transition-colors duration-200">
        <form action="{{ route('suppliers.update', $supplier->id) }}" method="POST" class="space-y-6" onsubmit="return confirm('Apakah Anda yakin ingin memperbarui data pemasok ini?');">
            @csrf
            @method('PUT')
            ...
            <div>
                ...
                <div class="mt-2 relative">
                    <input id="name" name="name" type="text" required value="{{ old('name', $supplier->name) }}"
                           ...
                           placeholder="Contoh: PT Indotex Utama">
                </div>
                @error('name')
            ...
            <div>
                ...
                <div class="mt-2 relative">
                    <textarea id="address" name="address" rows="4" required
                              ...
                              placeholder="Contoh: Jl. Industri Raya No. 45, Bandung, Jawa Barat">{{ old('address', $supplier->address) }}</textarea>
                </div>
                @error('address')
                    <p class="mt-2 text-xs text-rose-500 dark:text-rose-400 font-medium">{{ $message }}</p>
                @enderror
