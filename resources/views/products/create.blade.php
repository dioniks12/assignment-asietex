...
@section('page_title', __('Tambah Produk'))
...
<div class="max-w-xl mx-auto space-y-6">
    ...
    <!-- Header Actions -->
    <div>
        <h3 class="text-lg font-bold text-slate-800 dark:text-white">{{ __('Tambah Produk Baru') }}</h3>
        <p class="text-sm text-slate-500 dark:text-slate-400">{{ __('Tambahkan item katalog bahan baku baru ke dalam inventori.') }}</p>
    </div>
    ...
    <div class="bg-white dark:bg-slate-900 border border-slate-200 dark:border-slate-800 rounded-3xl p-6 lg:p-8 shadow-md dark:shadow-xl transition-colors duration-200">
        <form action="{{ route('products.store') }}" method="POST" class="space-y-6" data-confirm="{{ __('Apakah Anda yakin data produk ini sudah benar?') }}">
            @csrf
