...
@section('page_title', __('Produk Bahan Baku'))
...
<div class="space-y-6">
    ...
    <div class="flex flex-col sm:flex-row justify-between items-start sm:items-center gap-4">
        <div>
            <h3 class="text-lg font-bold text-slate-800 dark:text-white">{{ __('Daftar Inventori Produk') }}</h3>
            <p class="text-sm text-slate-500 dark:text-slate-400">{{ __('Lacak dan kelola item bahan baku beserta jumlah stok.') }}</p>
        </div>
    ...
    <div class="flex flex-col md:flex-row md:items-center justify-between gap-4 bg-white dark:bg-slate-900 border border-slate-200 dark:border-slate-800 p-4 rounded-2xl shadow-sm dark:shadow-md transition duration-200">
        <form action="{{ route('products.index') }}" method="GET" class="flex items-center w-full md:max-w-md gap-2">
            ...
            <div class="relative flex-1">
                ...
                <input type="text" name="search" value="{{ $search }}" placeholder="{{ __('Cari nama, warna, satuan, kategori...') }}" 
                            ...
                            <tr class="hover:bg-slate-50 dark:hover:bg-slate-800/40 transition duration-150">
                                ...
                                <td class="px-6 py-4 whitespace-nowrap text-right text-sm font-medium">
                                    <div class="flex justify-end space-x-2">
                                        <a href="{{ route('products.show', $product->id) }}" class="inline-flex items-center px-2.5 py-1.5 bg-slate-100 hover:bg-slate-200 dark:bg-slate-800 dark:hover:bg-slate-700 border border-slate-200 dark:border-slate-700 text-xs font-semibold text-slate-700 dark:text-slate-300 rounded-lg transition duration-200">
                                            {{ __('Detail') }}
                                        </a>
                                        @if(auth()->user()->role === 'manajer')
                                            ...
                                            <form action="{{ route('products.destroy', $product->id) }}" method="POST" class="inline-block delete-form" data-confirm="{{ __('Apakah Anda yakin ingin menghapus produk ini?') }}">
                                                @csrf
                                                @method('DELETE')
