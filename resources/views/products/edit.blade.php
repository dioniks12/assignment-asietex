...
@section('page_title', 'Ubah Produk')
...
<div class="max-w-xl mx-auto space-y-6">
    ...
    <!-- Header Actions -->
    <div>
        <h3 class="text-lg font-bold text-slate-800 dark:text-white">Ubah Produk</h3>
        <p class="text-sm text-slate-500 dark:text-slate-400">Ubah detail dan stok saat ini untuk Produk: {{ $product->name }}.</p>
    </div>
    ...
    <div class="bg-white dark:bg-slate-900 border border-slate-200 dark:border-slate-800 rounded-3xl p-6 lg:p-8 shadow-md dark:shadow-xl transition-colors duration-200">
        <form action="{{ route('products.update', $product->id) }}" method="POST" class="space-y-6" onsubmit="return confirm('Apakah Anda yakin ingin memperbarui data produk ini?');">
            @csrf
            @method('PUT')
