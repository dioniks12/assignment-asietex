...
<div class="max-w-xl mx-auto space-y-6">
    ...
    <div class="flex justify-between items-center">
        <div>
            <h3 class="text-lg font-bold text-slate-800 dark:text-white">Log Referensi Transaksi</h3>
            <p class="text-sm text-slate-500 dark:text-slate-400">ID: TX-{{ str_pad($transaction->id, 5, '0', STR_PAD_LEFT) }}</p>
        </div>
    ...
    <div class="bg-white dark:bg-slate-900 border border-slate-200 dark:border-slate-800 rounded-3xl p-6 lg:p-8 shadow-md dark:shadow-xl transition-colors duration-200 space-y-6">
        ...
        <div class="flex justify-between items-center border-b border-slate-200 dark:border-slate-800/80 pb-4">
            <div>
                <h4 class="text-xs font-semibold text-slate-500 dark:text-slate-450 uppercase tracking-wider">Tanggal & Waktu</h4>
            ...
            <div>
                ...
                ">
                    {{ strtoupper($transaction->type === 'in' ? 'Barang Masuk' : ($transaction->type === 'out' ? 'Barang Keluar' : 'Retur')) }}
                </span>
            </div>
        </div>
        ...
        <div class="grid grid-cols-2 gap-6">
            <!-- Product Item -->
            <div>
                <h4 class="text-xs font-semibold text-slate-500 dark:text-slate-450 uppercase tracking-wider">Nama Produk</h4>
            ...
            <div class="text-right">
                <h4 class="text-xs font-semibold text-slate-500 dark:text-slate-450 uppercase tracking-wider">Jumlah Mutasi</h4>
        ...
        <div class="border-t border-slate-200 dark:border-slate-800/80 pt-4">
            <h4 class="text-xs font-semibold text-slate-500 dark:text-slate-450 uppercase tracking-wider">Pemasok Terkait</h4>
            <p class="mt-1 text-sm font-bold text-slate-755 dark:text-slate-200">
                ...
                @else
                    <span class="text-slate-500 dark:text-slate-400 font-medium">Tidak ada (Penggunaan internal barang keluar)</span>
                @endif
            </p>
        </div>
        ...
        <!-- Notes -->
        <div class="border-t border-slate-200 dark:border-slate-800/80 pt-4">
            <h4 class="text-xs font-semibold text-slate-500 dark:text-slate-450 uppercase tracking-wider">Catatan Audit / Alasan</h4>
            <div class="mt-1 p-4 bg-slate-50 dark:bg-slate-950/60 border border-slate-200 dark:border-slate-800/80 rounded-2xl">
                <p class="text-xs text-slate-600 dark:text-slate-300 leading-relaxed italic">{{ $transaction->notes ?? 'Tidak ada catatan yang diberikan untuk log mutasi ini.' }}</p>
            </div>
        </div>
    </div>
