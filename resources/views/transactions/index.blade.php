...
@section('page_title', 'Log Mutasi Stok')
...
<div class="space-y-6">
    ...
    <div class="flex flex-col sm:flex-row justify-between items-start sm:items-center gap-4">
        <div>
            <h3 class="text-lg font-bold text-slate-800 dark:text-white">Log Mutasi Stok Gudang</h3>
            <p class="text-sm text-slate-500 dark:text-slate-400">Daftar lengkap barang masuk, keluar, dan retur bahan baku.</p>
        </div>
    ...
    <div class="flex flex-col md:flex-row md:items-center justify-between gap-4 bg-white dark:bg-slate-900 border border-slate-200 dark:border-slate-800 p-4 rounded-2xl shadow-sm dark:shadow-md transition duration-200">
        <form action="{{ route('transactions.index') }}" method="GET" class="flex items-center w-full md:max-w-md gap-2">
            ...
            <div class="relative flex-1">
                ...
                <input type="text" name="search" value="{{ $search }}" placeholder="Cari produk, pemasok, tipe, catatan..." 
        ...
        <div class="overflow-x-auto">
            @if($transactions->isEmpty())
                <div class="p-8 text-center text-slate-500">
                    Tidak ada transaksi yang tercatat dalam sistem.
                </div>
            @else
                        ...
                        @foreach($transactions as $transaction)
                            <tr class="hover:bg-slate-50 dark:hover:bg-slate-800/40 transition duration-150">
                                ...
                                <td class="px-6 py-4 whitespace-nowrap text-right text-sm font-medium">
                                    <a href="{{ route('transactions.show', $transaction->id) }}" class="inline-flex items-center px-2.5 py-1.5 bg-slate-100 hover:bg-slate-200 dark:bg-slate-850 dark:hover:bg-slate-800 border border-slate-200 dark:border-slate-700 text-xs font-semibold text-slate-700 dark:text-slate-300 rounded-lg transition duration-200">
                                        Detail
                                    </a>
                                </td>
                            </tr>
