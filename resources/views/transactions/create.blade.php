...
@section('page_title', 'Catat Transaksi')
...
<div class="max-w-xl mx-auto space-y-6">
    ...
    <!-- Header Actions -->
    <div>
        <h3 class="text-lg font-bold text-slate-800 dark:text-white">Catat Mutasi Stok</h3>
        <p class="text-sm text-slate-500 dark:text-slate-400">Catat pergerakan bahan masuk, keluar, atau retur. Jumlah stok akan diperbarui secara otomatis.</p>
    </div>
    ...
    <div class="bg-white dark:bg-slate-900 border border-slate-200 dark:border-slate-800 rounded-3xl p-6 lg:p-8 shadow-md dark:shadow-xl transition-colors duration-200">
        <form action="{{ route('transactions.store') }}" method="POST" class="space-y-6" onsubmit="return confirm('Apakah Anda yakin data transaksi ini sudah benar?');">
            @csrf
            ...
            <div>
                ...
                <div class="mt-2">
                    <select id="type" name="type" required
                        ...
                        <option value="in" {{ old('type') == 'in' ? 'selected' : '' }}>MASUK (Barang Masuk / Restock)</option>
                        <option value="out" {{ old('type') == 'out' ? 'selected' : '' }}>KELUAR (Barang Keluar / Pemakaian)</option>
                        <option value="return" {{ old('type') == 'return' ? 'selected' : '' }}>RETUR (Retur ke Pemasok)</option>
                    </select>
                </div>
            </div>
                    ...
                    <select id="product_id" name="product_id" required
                        ...
                        @foreach($products as $product)
                            <option value="{{ $product->id }}" data-unit="{{ $product->unit }}" data-stock="{{ $product->current_stock }}" {{ old('product_id') == $product->id ? 'selected' : '' }}>
                                {{ $product->name }} (Warna: {{ $product->color }} | Stok Tersedia: {{ $product->current_stock }} {{ $product->unit }})
                            </option>
                        @endforeach
                    </select>
                </div>
            ...
            <div id="supplier-wrapper" class="transition-opacity duration-200">
                ...
                <p class="mt-1 text-xs text-slate-500 font-medium">Wajib diisi untuk tipe transaksi MASUK dan RETUR.</p>
            </div>
            ...
            <div>
                ...
                <div class="mt-2">
                    <textarea id="notes" name="notes" rows="3"
                              class="block w-full px-4 py-3 bg-white dark:bg-slate-950/80 border border-slate-300 dark:border-slate-800 focus:ring-indigo-500 focus:border-indigo-500 rounded-xl text-slate-900 dark:text-slate-100 placeholder-slate-400 dark:placeholder-slate-500 focus:outline-none focus:ring-2 transition duration-200"
                              placeholder="Contoh: Restock benang katun, perintah produksi #104">{{ old('notes') }}</textarea>
                </div>
            </div>
